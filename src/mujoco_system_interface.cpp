    ...
    const std::vector<std::string> & stop_interfaces)
{
  ...
  auto update_joint_interface = [this](const std::string& interface_name, bool enabled)
  {
    ...
    if (delimiter_pos == std::string::npos) {
      RCLCPP_ERROR(rclcpp::get_logger("MujocoSystemInterface"), "Invalid interface name format: %s", interface_name.c_str());
      return;
    }
    ...
    {
      RCLCPP_WARN(rclcpp::get_logger("MujocoSystemInterface"), "Joint %s not found in joint_states_", joint_name.c_str());
      return;
    }
    ...
    else
    {
      ...
      RCLCPP_INFO(rclcpp::get_logger("MujocoSystemInterface"),
                  "Joint %s: %s control disabled", joint_name.c_str(), interface_type.c_str());
    }
  };
